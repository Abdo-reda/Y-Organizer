...
// - save svg as png, FIX FIX FIX FIX
...
function saveSvg() {
	...
	console.log("--- svg", drawArea.value?.outerHTML);
...
function saveSvgAsPng() {
	...
	doodleImage.onload = () => {
		if (!canvasDrawer.value || !fileSaver.value || !drawArea.value) return;
		canvasDrawer.value.width = drawArea.value.width.baseVal.value;
		canvasDrawer.value.height = drawArea.value.height.baseVal.value;
