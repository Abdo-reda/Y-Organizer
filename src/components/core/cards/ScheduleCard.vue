	...
	<FunctionCard title="Schedule">
		<template #default>
			<Popover v-model:open="editSessionPopoverOpen">
				...
				<SessionFormPopover :existing-session="editSession" @create="handleSessionCreate" @update="handleSessionUpdate" />
			</Popover>
			<Popover v-model:open="previewSessionPopoverOpen">
				<PopoverAnchor :reference="sessionPopoverAnchor"> </PopoverAnchor>
				<SessionPreviewPopover :session="editSession" :tasks="tasks" />
			</Popover>
			<div class="flex h-full select-none">
				...
				<div @contextmenu.prevent ref="schedule-container" class="!h-full grid-stack flex-1 relative mx-0.5">
					<div class="absolute size-full z-10 pointer-events-none p-0.5">
						<div class="w-full bg-primary/10 backdrop-saturate-50 rounded-t-sm" :style="{ height: `${dayPercentage * 100}%` }" />
						<div class="w-full border-foreground/20 border-t-2" />
					</div>
					<StepForwardIcon class="absolute stroke-transparent fill-primary -translate-y-1/2 -translate-x-full size-3" :style="{ top: `${dayPercentage * 100}%` }" />
					<div class="absolute h-full w-full flex flex-col p-0.5 gap-1.5">
						<div v-for="i in 24" :key="i" @click="openSessionPopover($event, i - 1)" class="flex-1 group">
							...
							>
								<PlusIcon class="size-3 text-muted group-hover:text-muted-foreground transition-colors group-hover:transition-none" />
							</div>
						</div>
					</div>
					<template v-for="session in sessions" :key="session.id">
						<div
							...
							:gs-x="0"
							:gs-y="session.startTime.hour"
							:gs-h="session.endTime.diff(session.startTime, 'hours').hours"
							:gs-id="`s${session.id}`"
							:id="`s${session.id}`"
							:style="{ '--color-hover': activities.find((a) => a.name === session.activity)?.color ?? 'var(--primary)' }"
						>
							<div
								@contextmenu="handleSessionDelete(session.id)"
								@click="handleSession($event, session)"
