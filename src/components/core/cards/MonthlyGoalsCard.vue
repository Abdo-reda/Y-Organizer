...
const backlogGoals = computed(() =>
	monthlyGoals.filter((g) => g.status !== GoalStatusEnum.ACTIVE && g.completedDay !== selectedDay.value.toISODate({ precision: "month" })).sort((a, b) => b.status.localeCompare(a.status))
);
const currentMonthGoals = computed(() =>
	monthlyGoals.filter((g) => g.status !== GoalStatusEnum.DISABLED && (g.completedDay === selectedDay.value.toISODate({ precision: "month" }) || !g.completedDay))
);
