...
function handleTaskPrimary(event: MouseEvent, task: ITask) {
	if (event.ctrlKey) openEditDialog(task);
	else if (event.altKey) setTaskToSession(task);
}

function setTaskToSession(task: ITask) {
    if (!currentSession.value?.id) return;
    if (currentSession.value.activity === task.activity && currentSession.value.id !== task.session) {
        updateTask(task.id, {...task, session: currentSession.value.id, isToday: true})
    }
}
				...
				<CarouselContent @contextmenu.prevent>
					<CarouselItem>
						<div class="flex flex-col h-full">
							...
							<TransitionGroup ref="tasks-container" name="auto" tag="ul" class="scroll-drag flex-1 select-none space-y-2 p-2">
								<template v-if="todayTasks.length">
									...
									>
										...
										<div class="flex-1 min-w-0">
											...
											<div v-if="task.status !== TaskStatusEnum.COMPLETED" class="text-xs text-muted-foreground mt-1 truncate">
					...
					<CarouselItem>
						<div class="flex flex-col h-full">
							...
							<TransitionGroup ref="backlog-container" name="auto" tag="ul" class="scroll-drag flex-1 select-none space-y-2 p-2">
								<li
