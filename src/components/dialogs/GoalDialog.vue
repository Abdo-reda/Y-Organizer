...
const goalForm = reactiveComputed<IGoal>(() => (props.existingGoal ? { ...props.existingGoal } : getDefaultGoal()));
		...
		<form id="goal-form" name="goal-form" @submit.prevent="handleSubmit">
			<div class="flex flex-col gap-3">
							...
							<SelectTrigger class="w-full">
								<SelectValue>
									<p class="flex gap-2 items-center" v-if="selectedActivity">
										<span class="rounded-full block size-3" :style="{ backgroundColor: selectedActivity.color }"> </span>
										{{ selectedActivity.name }}
									</p>
									<p v-else>Select Activity...</p>
								</SelectValue>
							</SelectTrigger>
							<SelectContent>
								<SelectGroup>
									<SelectItem v-for="activity in activeActivities" :value="activity" class="focus:bg-hover/10" :style="{ '--color-hover': activity.color }">
										<span class="rounded-full size-3" :style="{ backgroundColor: activity.color }"> </span> {{ activity.name }}
									</SelectItem>
								</SelectGroup>
							</SelectContent>
						</Select>
				...
				<div class="flex flex-col gap-1.5">
					...
					<Textarea id="description" v-model="goalForm.description" placeholder="Goal description..." class="resize-none" />
				</div>
			</div>
		</form>
		<DialogFooter>
			<Button type="submit" variant="outline" form="goal-form"> <PenIcon /> {{ props.existingGoal ? "Update" : "Create" }} </Button>
		</DialogFooter>
	</DialogContent>
</template>
