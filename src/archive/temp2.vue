      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="text-center mb-6">
            <h2 class="text-lg font-medium text-gray-600 mb-2">Now</h2>
          ...
          <div v-if="currentActivity?.currentTask" class="mb-4">
            <div class="flex items-center justify-center gap-2">
              <div class="relative">
                <div class="w-2 h-2 rounded-full animate-ping" :style="{ backgroundColor: currentActivity.color }"></div>
                <div class="absolute inset-0 w-2 h-2 rounded-full" :style="{ backgroundColor: currentActivity.color }"></div>
              </div>
          ...
          <div v-if="currentActivity?.tasks?.length" class="mt-4">
            <div class="flex items-center justify-between mb-2">
              ...
              <button
                @click="showAddActivityTask = true"
            ...
            <div v-show="showOtherTasks" class="space-y-1">
              <div 
                v-for="task in currentActivity.tasks" 
                :key="task.id"
                ...
                @click="setCurrentTask(task.title)"
                draggable="true"
                @dragstart="draggedTask = task"
                @dragover.prevent
                @drop="setCurrentTask(task.title)"
              >
              ...
              >
                ...
                <svg class="w-32 h-32 transform -rotate-90" viewBox="0 0 36 36">
                  ...
                  <path
                    ...
                    d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                    fill="none"
                    :stroke="isBreakActive ? 'url(#breakGradient)' : (currentActivity ? `url(#activityGradient)` : 'url(#pomodoroGradient)')"
                    stroke-width="3"
                ...
                <!--  Updated SVG gradient definitions to include activity-based gradient -->
                <svg width="0" height="0">
                  <defs>
                    ...
                    <linearGradient v-if="currentActivity" id="activityGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" :style="`stop-color:${currentActivity.color};stop-opacity:1`" />
                      <stop offset="100%" :style="`stop-color:${adjustColorBrightness(currentActivity.color, -20)};stop-opacity:1`" />
                    </linearGradient>
                  </defs>
                </svg>
              </div>
